...
public class DumpAction extends ActionSupport{
	...
	BaseDao<Dump> baseDao=new DumpDao();
	...
	public String save()throws Exception{
		try {
			int saveNums=0;
			JSONObject result=new JSONObject();
			if(StringUtil.isNotEmpty(dumpId)){
				dump.setId(Integer.parseInt(dumpId));
				saveNums=baseDao.update(dump);
			}else{
				saveNums=baseDao.insert(dump);
			}
			if(saveNums>0){
