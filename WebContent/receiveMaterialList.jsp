...
function review(id) {
	$.ajax({
		type:'get',
		url:'repertory/receiveOrder!receive',
		data:{
			receiveOrderId:id
		},
		dataType:'json',
		success:function(data) {
			if(data.result != 'success'){
				$.messager.alert("系统提示",'收料失败');
			}else{
				$.messager.alert("系统提示","收料成功");
				$("#dg").datagrid("reload");
			}
		},
		error:function(data){
...
var formatReview = function(value,rawData,index) {
	if (rawData.isreceive == "未收料")　{
		return '<a href="#" onclick="review(\'' + rawData.id + '\')" class="rw-btn">收料</a>';
	} else {
		return value;
	}
}
...
<body>
<table id="dg" title="已审核采购订单信息" class="easyui-datagrid" fitColumns="true"
	 pagination="true" rownumbers="true" url="repertory/receiveOrder!reviewList" fit="true">
	<thead>
		<tr>
			...
			<th data-options="field:'isreceive',width:200,align:'center',formatter:formatReview">收料</th>
		</tr>
	</thead>	
</table>
</body>
