...
function deleteInventory(){
	...
	$.messager.confirm("系统提示","您确认要删掉这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
		if(r){
			$.post("repertory/finishedInventory!delete",{delIds:ids},function(data){
...
function openInventoryAddDialog(){
	...
	url="repertory/finishedInventory!save";
}
...
function openInventoryModifyDialog(){
	...
	// $("#fm").form("load",row);
	$("#dlg_materialNumber").val(row.finishedmaterialnumber);
	$("#dlg_count").val(row.restcount);
	url="repertory/finishedInventory!save?finishedInventoryId="+row.id;
}
...
<body>
<table id="dg" title="成品库存信息" class="easyui-datagrid" fitColumns="true"
	 pagination="true" rownumbers="true" url="repertory/finishedInventory" fit="true" toolbar="#tb">
	<thead>
		<tr>
			<th data-options="field:'cb',checkbox:true"></th>
			<th data-options="field:'finishedmaterialnumber',width:50,align:'center'">成品编码</th>
			<th data-options="field:'restcount',width:100,align:'center'">成品库存量</th>
		</tr>
	</thead>	
</table>
<div id="dlg" class="easyui-dialog" style="width:590px;height:350px;padding:10px 20px" closed="true" buttons="#dlg-button">
	<form action="" method="post" id="fm">
		<table cellspacing="5px">
			<tr>
				<td >成品编码:</td>
				<td><input type="text" name="finishedInventory.finishedMaterialNumber" id="dlg_materialNumber" class="easyui-validatebox" required="true"/></td>
			</tr>
			<tr>
				<td>成品库存量:</td>
				<td><input type="text" name="finishedInventory.restCount" id="dlg_count" class="easyui-validatebox" required="true"/></td>
			</tr>
		</table>
	</form>
</div>
...
<div id="tb">
	...
	<div>
		成品编码:<input type="text" name="materialNumber" id="materialNumber" /><a href="javascript:searchInventory()" class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
	</div>
</div>
</body>
</html>